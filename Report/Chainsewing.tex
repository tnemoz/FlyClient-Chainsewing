...
Starting from \(\mathsf{m}=\mathsf{f+j+1}\), which we will call the \textit{merging block} from now on, every block mined by the adversary will contain a MMR root corresponding to the MMR where the portion \(\mathsf{C[}f+1:f+j+1\mathsf{]}\) has been replaced with the adversary's fork, and where all honest miner's blocks are considered as legacy blocks. Once in this situation, a number \(n-m>k\) of blocks are mined on top of \(\mathsf{C[}f+j+1\mathsf{]}\). In order to have, let us say, \(\mathsf{C[}f+1\mathsf{]}'\) accepted, the adversary must:
      ...
      \begin{figure}[ht]
        ...
        \begin{tikzpicture}[decoration = {snake}]
          ...
          \node[below of=cprime1] {\(\Cindex{f+1}'\) };
          \node[block, below right=of c] (c1) {};
          \node[block, above right=of cprime1, fill=black] (cprime2bis) {};
          \node[below of=cprime2bis] {\(\Cindex{f+2}'\) };
          \node[block, below right=of cprime1] (cprime2) {};
      ...
      \begin{itemize}
              \item if \(\Cindex{m}\) is not sampled, there is no problem;
              \item if \(\Cindex{m}\) is sampled, then the adversary has to build a MMR where the hash at place \(f+2\) is the hash of \(\Cindex{f+2}'\).
      \end{itemize}

      % TODO: Explain why the merging block can't just wait for blocks to be mined and declare them as legacy (because they would have to send them all, can't send (f+1)' in this case)
      % TODO: Verify this parti, it is not clear
      The adversary wants to convince the verifier that \(\Cindex{f+1}'\) is within the chain, but the have no control over the values of the leaves number \(f+1\), which contains the hash of \(\Cindex{f+1}'\) and \(f+2\), which contains the hash of \(\Cindex{f+2}\). Indeed, as a recall, their goal is that there is no inconsistency between their previous block reference and their MMR root. For this reason, it is necessary that leaf number \(f+2\) contains the hash of \(\Cindex{f+2}\).   Hence, the only thing they has control on is the MMR proof they send. What the adversary has to do is to create a MMR proof so that the hashes they send add up to the MMR root in \(\Cindex{m}\). However, it means that the adversary has to find a hash so that everything adds up correctly to the MMR root. Since the hash function used is believed to be pre-image resistant, this is computationally infeasible. Plus, a sampling where both \(\Cindex{m}\) and \(\Cindex{f+2}\) are sampled would also reveal an inconsistency.
